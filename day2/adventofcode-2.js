...
function resolve(input){
    ...
    return input.filter((check) => {
        let pass = [...check.pass.trim()];
        let times = pass.filter(letter => { 
            return letter == check.letter 
        }).length;
        return check.min <= times && times <= check.max;
    })
}
...
function resolve2(input){
    ...
    return input.filter((check) => {
        let pass = [...check.pass];
        let pos1 = parseInt(check.min);
        let pos2 = parseInt(check.max);
        return (pass[pos1] == check.letter || pass[pos2] == check.letter) && !(pass[pos1] == check.letter && pass[pos2] == check.letter);
    })
    
}

function inputObj(input){
    return input.map((register) => {
        let parts = register.split(':');
        let policy = parts[0].split(' ');
...
console.log(output.length) // 396

const output2 = resolve2(input);

console.log(output2.length) // 428