...
function parseInput(inputText) {
    let passports = inputText
    .trim()
    .split("\n\n");
    let maybeValidPassports = passports.map((passport) => {
        passport = passport.match(/\S+/g)
        return passport.map(p => {
            let data = p.split(':');
            return data;
        })
    })
        //return p.length == 8 || (p.length == 7 && p.find(val => !val.cid))
    // console.log({maybeValidPassports})
    return maybeValidPassports;
}
function resolve(input){
    let out = input.filter(passport => {
        if( passport.length == 8 
            || (passport.length == 7 && !passport.find(reg => reg[0] === 'cid')) 
        ){
            return true;
        }
    })
...
function resolve2(input){
    let out = input.filter(passport => {
        if( passport.length == 8 
            ...
            if (
                ...
                checkByr( checkData(passport, 'byr') )
                &&
                checkIyr( checkData(passport, 'iyr') )
                &&
                checkEyr( checkData(passport, 'eyr') )
                &&
                checkHgt( checkData(passport, 'hgt') )
                &&
                checkHcl( checkData(passport, 'hcl') )
                &&
                checkEcl( checkData(passport, 'ecl') )
                &&
                checkPid( checkData(passport, 'pid') )
            ){
                return true;
            }
        }
    ...
    function checkData(passport, field){
        let data = passport.find(reg => reg[0] === field);
        return data[1];
    }
    // byr (Birth Year) - four digits; at least 1920 and at most 2002.
    function checkByr(data){
        return data.length === 4 && data >= 1920 && data <= 2002;
    }
    // iyr (Issue Year) - four digits; at least 2010 and at most 2020.
    function checkIyr(data){
        return data.length === 4 && data >= 2010 && data <= 2020;
    }
    // eyr (Expiration Year) - four digits; at least 2020 and at most 2030.
    function checkEyr(data){
        return data.length === 4 && data >= 2020 && data <= 2030;
    }
    // hgt (Height) - a number followed by either cm or in:
        // If cm, the number must be at least 150 and at most 193.
        // If in, the number must be at least 59 and at most 76.
    function checkHgt(data){
        let h = data.substr(0, data.length-2);
        let measure = data.substr(-2);
        return (['cm', 'in'].includes(measure) 
            && (
                (measure === 'cm' && h >= 150 && h <= 193) 
                || (measure === 'in' && h >= 59 && h <= 76)
            )
        )
    }

    // hcl (Hair Color) - a # followed by exactly six characters 0-9 or a-f.
    function checkHcl(data){
        return /^#([a-fA-F0-9]{6})/.test(data);
    }
    // ecl (Eye Color) - exactly one of: amb blu brn gry grn hzl oth.
    function checkEcl(data){
        return ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].includes(data);
    }
    // pid (Passport ID) - a nine-digit number, including leading zeroes.
    function checkPid(data){
...
console.log(output) // 202

const output2 = resolve2(input);

console.log(output2) // 137