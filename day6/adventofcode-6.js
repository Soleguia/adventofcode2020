...
function uniqueAnswers(value, index, self){
    // Checks if the given value is the first occurring. 
    // If not, it must be a duplicate and will not be copied.
    return self.indexOf(value) === index;
}
function groupTotalAnswers(group){
    return group.reduce(
        (totalAnswers, answers) => [...totalAnswers, ...answers]
    )
}
function resolve(input){
    let totalAnswers = input.map(group => {
        let groupAnswers        = groupTotalAnswers(group);
        let groupUniqueAnswers  = groupAnswers.filter(uniqueAnswers);
        return groupUniqueAnswers.length;
    })
    return totalAnswers.reduce((accum, answer) => accum + answer);
}

function resolve2(input){
    let groupCommonAnswers = input.map(group => {
        let groupAnswers        = groupTotalAnswers(group);
        let groupUniqueAnswers  = groupAnswers.filter(uniqueAnswers);
        let commonAnswers       = groupUniqueAnswers.filter(answer => {
            let common = groupAnswers.filter( answr => {
                return answr === answer;
            })
            if(common.length === group.length){
                return answer;
            }
        })
        return commonAnswers.length;
    })
    return groupCommonAnswers.reduce((accum, answer) => accum + answer);
}

const input  = parseInput(inputText)
const output = resolve(input);

console.log(output) // 6903

const output2 = resolve2(input);

console.log(output2) // 3493